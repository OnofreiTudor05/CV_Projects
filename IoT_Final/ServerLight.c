...
#define PORT_CONECTARE 2909               // portul folosit pentru listening
...
{
  system("clear");
  int fisier_config = open("_resources/_Configurare/_config_light.txt", O_RDONLY);  // citire stare
  ...
  int fileConfig = open("_resources/_Configurare/_config_light.txt", O_WRONLY | O_TRUNC);
...
{		
    ...
    int fileConfig = open("_resources/_Configurare/_config_light.txt", O_WRONLY | O_TRUNC);          // la terminarea threadului, il inchidem si salvam starea rezultata de modificarile aduse de client
    write(fileConfig, Stare.volum, sizeof(Stare.volum));
    close(fileConfig);
