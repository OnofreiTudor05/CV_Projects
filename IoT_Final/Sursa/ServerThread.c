...
DateThread VectorThreads[100];

int main()
{
  system("clear");
  int fisier_config = open("_config_light.txt", O_RDONLY);  // citire stare

  char numar[255];
  char mesajConectare[255];
  bzero(numar, 255);
  ...
  while(1)
  {
    bzero(mesajConectare, 255);

    int client;
    DateThread* threadNou;                                                   
    ...
    sprintf(mesajConectare, "%s%s", "Stare curenta: ", Stare.volum);
    printf("Mesaj CON: %s\n", mesajConectare);
    write(client, mesajConectare, 255);     
	  ...
	  VectorThreads[i].ID = i;
    VectorThreads[i].DescriptorClient = client;                                        // salvam in thread descriptorul clientului          
...
{		
    ...
    for(int ii=0; ii<i-1; ii++){
      threads[ii] = threads[ii+1];
      VectorThreads[ii] = VectorThreads[ii+1];
    }
    i--;
...
void trimiteRaspuns(void *arg, char* raspunsSalvat)
{
  ...
  for(int ii=0; ii<i; ii++){
        if(write(VectorThreads[ii].DescriptorClient, sir, sizeof(sir)) <= 0)
        {
          printf("[Thread %d] %s",threadDeProcesat.ID, "Eroare la scriere.");
