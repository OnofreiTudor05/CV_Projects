...
int main (int argc, char *argv[])
{
  ...
  while(okConectat){                                                      // cat timp clientul decide sa trimita cereri
    bzero(MesajCitit, 255);
    bzero(bufferMesaj, 255);
    ...
    scanf("%s", MesajCitit);                                             // se citeste cererea in buffer            
    if(write(socketDescriptor, MesajCitit, 255) <= 0)                    // daca nu s-a reusit scrierea                       
    {
      perror("[client]Eroare la write() spre server.\n");                 // afisam un mesaj de eroare si inchidem programul 
      return errno;
    }
    if(!strcmp(MesajCitit, "exit")){                                             // daca clientul a decis sa se deconecteze
